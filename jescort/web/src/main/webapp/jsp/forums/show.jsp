...
<div id="body">
        ...
        <ol class="breadcrumb top ipsList_inline">
            <li itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb">
                <a href="<spring:url value="/"/>" itemprop="url">
                    <span itemprop="title">${category.subject}</span>
                </a>
            </li>
            <li itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb">
    ...
    <div class="topic_controls clearfix">
        ...
        <ul class="topic_buttons">
            <shiro:authenticated>
                <li><a href="<spring:url value="/forums/${forum.id}/topics/new"/>" title="<spring:message code="message.start_new_topic"/>" accesskey="s"><spring:message code="message.start_new_topic"/></a></li>
            </shiro:authenticated>
                    ...
                    <c:forEach var="topic" items="${topics.content}" varStatus="status">
                        <tr class="__topic<c:if test="${status.index % 2 == 1}"> unread</c:if> expandable" id="trow_${topic.id}" data-tid="${topic.id}">
                            <td class="col_f_icon altrow short">
                                <a href="<spring:url value="/topic/${topic.id}"/>" title="Go to first page">
                            ...
                            <td class="col_f_preview __topic_preview">
                                <a href="<spring:url value="/topic/${topic.id}"/>" class="expander closed" title="Preview this topic">&nbsp;</a>
                            </td>
