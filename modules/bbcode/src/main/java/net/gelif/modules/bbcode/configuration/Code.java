...
public final class Code
{
     ...
     * @param createdScopes
     *            scopes are created already
     * @param codes
     *            codes are created already
     * @return code object
     */
    AbstractCode create(Configuration configuration, Map<Scope, WScope> createdScopes, Map<Code, AbstractCode> codes)
    {
        ...
        if(code == null)
        {
            ...
            {
                code = new ConstantCode(((Constant)first).getValue(), getTemplate().create(), getName(), getPriority());
            }
            else
            {
                code = new WCode(getPattern().create(configuration, createdScopes, codes), getTemplate().create(), getName(), getPriority());
            }
        }
        return code;
    }
